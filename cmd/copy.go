...
func init() {
	InitCommand(
		...
		func(c *cobra.Command, f *flag.FlagSet) {
			...
			f.StringVar(&cargs.destinationPassword, "destination-password", "", "Destination database username if required.")
			f.IntVarP(&cargs.maxParallelCollections, "maximum-concurrent-cursors", "m", 5, "Maximum number of cursors being read at in parallel.")
			...
			f.StringSliceVar(&cargs.excludedDatabases, "exluded-databases", []string{}, "A list of database names which should be excluded.")
			f.StringSliceVar(&cargs.excludedCollections, "excluded-collections", []string{}, "A list of collections names which should be excluded.")
			f.StringSliceVar(&cargs.excludedViews, "excluded-views", []string{}, "A list of view names which should be excluded.")
